  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`grid ${wide ? 'grid-cols-4' : 'grid-cols-2'} gap-3`}>
        ...
        <KpiCard label="Records" value={String(data.length)} color="blue" />
        <KpiCard label="Teams" value={String(Object.keys(byTeam).length)} color="green" />
      </div>
        ...
        <Card title="Shorts by Team" badge="Cost">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={teamData} margin={{ left: 0, right: 8, top: 4, bottom: 4 }}>
              ...
              <XAxis dataKey="name" tick={{ fill: '#9aa0b0', fontSize: 9 }} angle={-20} textAnchor="end" height={50} axisLine={false} />
              <YAxis tick={{ fill: '#9aa0b0', fontSize: 10 }} tickFormatter={v => '$' + v} axisLine={false} />
              <Tooltip
        ...
        <Card title="Top Partners">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={partnerData} layout="vertical" margin={{ left: 8, right: 16, top: 4, bottom: 4 }}>
              ...
              <XAxis type="number" tick={{ fill: '#9aa0b0', fontSize: 10 }} axisLine={false} />
              <YAxis type="category" dataKey="name" tick={{ fill: '#9aa0b0', fontSize: 9 }} width={100} axisLine={false} />
              <Tooltip
                contentStyle={{ background: '#1a1d27', border: '1px solid #2a2e3d', borderRadius: 8, fontSize: 12 }}
            ...
            <thead>
              <tr>
                {['Date', 'Product', 'Description', 'Partner', 'Shorts', 'Cost', 'Team'].map(h => (
                  <th key={h} className="sticky top-0 bg-[#1e2230] text-left p-2 text-[9px] font-semibold uppercase tracking-wider text-text-secondary border-b border-border whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filteredData.map((r, i) => (
                <tr key={i} className="border-b border-border/40 hover:bg-card-hover transition-colors">
                  ...
                  <td className="p-2 max-w-[200px] truncate">{String(r['DESCRIPTION'] || '').substring(0, 35)}</td>
